  ...
  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 bg-white/70 backdrop-blur-xl border border-gray-200/50 rounded-full px-6 py-3 shadow-sm flex items-center gap-6">
      <div className="flex items-center gap-8">
        {['About', 'Experience', 'Projects'].map((item) => (
          <button key={item} onClick={() => {
            ...
            else window.location.href = `/#${item.toLowerCase()}`;
          }} className="text-sm font-medium text-gray-500 hover:text-black">{item}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <button onClick={onLoginClick} className="text-gray-400 hover:text-black flex items-center gap-2">
          <LogIn size={18} />
              ...
              {user && role !== 'admin' && !authLoading && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-white border border-gray-200 px-8 py-6 rounded-[2rem] shadow-2xl flex flex-col items-center gap-4 max-w-md w-full animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="w-full bg-gray-50 p-4 rounded-xl space-y-2">
                    ...
                    <div className="flex justify-between text-[10px] font-mono text-gray-400">
                      <span>UID:</span>
                      <span className="text-gray-600 truncate ml-4">{user.uid}</span>
                    </div>
                  </div>
                  <div className="flex gap-3 w-full">
                    <button onClick={() => logout()} className="flex-1 bg-black text-white py-3 rounded-xl font-bold text-sm">登出帳號</button>
